...
Сервис Filmorate работает с фильмами и оценками пользователей, а также возвращает топ-5 фильмов, рекомендованных к
...
Бизнес-логика объединена в пакете service. 
#### 1. Класс UserService, дополнительно отвечает за такие операции с пользователями, как:
...
#### 2. Класс FilmService, отвечает за операции с фильмами:
- Добавление лайка. Каждый пользователь может поставить лайк фильму только один раз.
- Удаление лайка.
- Вывод 10 наиболее популярных фильмов по количеству лайков, с возможностью фильтрации по идентификатору жанра и/или году создания фильма.
- Получение списка фильмов которым поставили лайки одновременно и пользователь, отправляющий запрос, и другой пользователь. 
...
Логирование
Логирование осуществляется для операций, которые изменяют сущности — добавляют и обновляют их. Также логируются причины ошибок — например, если валидация не пройдена. 

Тестирование
Тесты для валидации. Подтверждают, что она работает на граничных условиях.
